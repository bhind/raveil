...
fixture_overlay="$repo_root/hardware/chisel/chipyard-overlay/RaveilFixtureInputProvider.scala"
static_core_overlay="$repo_root/hardware/chisel/chipyard-overlay/RaveilStaticStencilCore.scala"
static_client_overlay="$repo_root/hardware/chisel/chipyard-overlay/RaveilStaticStencilTLClient.scala"
...
boom_token_strip_patch="$repo_root/hardware/chisel/chipyard-patches/t-0042-boom-token-strip-after-valid.patch"
...
boom_store_token_stripped_verifier="$repo_root/hardware/chisel/verify_owned_boom_store_token_stripped_after_valid.py"
...
fixture_repeated_workload_c="$repo_root/hardware/chisel/riscv_stencil_fixture_repeated.c"
...
case "$cpu_mode:$cpu_config:$cpu_config_fq:$cpu_label:$build_volume" in
    ...
    boom-store-token-stripped-after-valid:RaveilOwnedSmallBoomTokenConfig:chipyard.raveil.RaveilOwnedSmallBoomTokenConfig:boom:raveil-chipyard-owned-boom-store-token-stripped-after-valid-build-v3) ;;
...
case "$cpu_mode" in
    ...
    boom-store-token-default-invalid)
        ...
        ;;
    boom-store-token-stripped-after-valid)
        applied_patch_manifest="$applied_patch_manifest,t-0042-boom-store-authorization.patch,t-0042-boom-store-token-handoff.patch,t-0042-boom-token-strip-after-valid.patch"
        ;;
...
for input in "$overlay" "$origin_overlay" "$fixture_overlay" "$static_core_overlay" "$static_client_overlay" "$rocket_hook_patch" "$rocket_witness_patch" "$rocket_fate_patch" "$rocket_exception_patch" "$boom_hook_patch" "$boom_lifecycle_patch" "$boom_misaligned_patch" "$boom_store_patch" "$boom_store_token_patch" "$boom_token_fields_only_patch" "$boom_token_strip_patch" "$boom_redirect_patch" "$xbar_request_patch" "$tl_token_patch" "$workload" "$verifier" \
...
fixture_overlay_sha256=$(shasum -a 256 "$fixture_overlay" | awk '{print $1}')
static_core_overlay_sha256=$(shasum -a 256 "$static_core_overlay" | awk '{print $1}')
static_client_overlay_sha256=$(shasum -a 256 "$static_client_overlay" | awk '{print $1}')
...
boom_token_strip_patch_sha256=$(shasum -a 256 "$boom_token_strip_patch" | awk '{print $1}')
...
input_sha256=$(
    {
        shasum -a 256 "$overlay" "$origin_overlay" "$fixture_overlay" "$static_core_overlay" "$static_client_overlay" "$rocket_hook_patch" "$rocket_witness_patch" "$rocket_fate_patch" "$rocket_exception_patch" "$boom_hook_patch" "$boom_lifecycle_patch" "$boom_misaligned_patch" "$boom_store_patch" "$boom_store_token_patch" "$boom_token_fields_only_patch" "$boom_token_strip_patch" "$boom_redirect_patch" "$xbar_request_patch" "$tl_token_patch" "$workload" \
            "$rocket_witness_workload" "$rocket_witness_verifier" "$rocket_redirect_workload" "$rocket_redirect_verifier" "$rocket_redirect_fate_verifier" \
            "$rocket_exception_workload" "$rocket_exception_verifier" "$boom_lifecycle_workload" "$boom_lifecycle_verifier" "$boom_misaligned_workload" "$boom_misaligned_verifier" "$boom_store_workload" "$boom_store_verifier" "$boom_store_token_verifier" "$boom_store_token_default_invalid_verifier" "$boom_store_token_stripped_verifier" "$boom_redirect_workload" "$boom_redirect_verifier" \
...
cache_source_sha256=$(
    {
        shasum -a 256 "$overlay" "$origin_overlay" "$fixture_overlay" "$static_core_overlay" "$static_client_overlay" "$rocket_hook_patch" "$rocket_witness_patch" "$rocket_fate_patch" "$rocket_exception_patch" "$boom_hook_patch" "$boom_lifecycle_patch" "$boom_misaligned_patch" "$boom_store_patch" "$boom_store_token_patch" "$boom_token_fields_only_patch" "$boom_token_strip_patch" "$boom_redirect_patch" \
...
if [ "$cpu_mode" = controlled ]; then
    source_sha256=$(
        {
            shasum -a 256 "$overlay" "$origin_overlay" "$fixture_overlay" "$static_core_overlay" "$static_client_overlay" "$rocket_hook_patch" \
...
if [ "$cpu_mode" = controlled-fixture-repeat ]; then
    source_sha256=$(
        {
            shasum -a 256 "$overlay" "$origin_overlay" "$fixture_overlay" "$static_core_overlay" "$static_client_overlay" \
                "$rocket_hook_patch" "$boom_hook_patch" \
                "$xbar_request_patch" "$tl_token_patch" \
...
docker run --rm \
    ...
    --env "RAVEIL_FIXTURE_OVERLAY_SHA256=$fixture_overlay_sha256" \
    --env "RAVEIL_STATIC_CORE_OVERLAY_SHA256=$static_core_overlay_sha256" \
    --env "RAVEIL_STATIC_CLIENT_OVERLAY_SHA256=$static_client_overlay_sha256" \
    ...
    --env "RAVEIL_BOOM_TOKEN_STRIP_PATCH_SHA256=$boom_token_strip_patch_sha256" \
...
if [ "$RAVEIL_OWNED_CPU_MODE" = debug-sba ] ||
   [ "$RAVEIL_OWNED_CPU_MODE" = controlled ] ||
	   ...
	   [ "$RAVEIL_OWNED_CPU_MODE" = controlled-fixture-repeat ] ||
...
if [ ! -e "$build_root" ]; then
  ...
  [ "$(sha256sum /repo/hardware/chisel/chipyard-patches/t-0042-boom-token-strip-after-valid.patch | awk "{print \$1}")" = "$RAVEIL_BOOM_TOKEN_STRIP_PATCH_SHA256" ]
  ...
  if [ "$RAVEIL_OWNED_CPU_MODE" = boom-load-lifecycle ]; then
    ...
    git -C "$build_root/chipyard/generators/boom" apply \
      /repo/hardware/chisel/chipyard-patches/t-0042-boom-misaligned-rollback.patch
       ...
       [ "$RAVEIL_OWNED_CPU_MODE" = boom-store-token-handoff ] ||
       [ "$RAVEIL_OWNED_CPU_MODE" = boom-store-token-default-invalid ] ||
       [ "$RAVEIL_OWNED_CPU_MODE" = boom-store-token-stripped-after-valid ]; then
    git -C "$build_root/chipyard/generators/boom" apply --check \
      /repo/hardware/chisel/chipyard-patches/t-0042-boom-store-authorization.patch
    git -C "$build_root/chipyard/generators/boom" apply \
      /repo/hardware/chisel/chipyard-patches/t-0042-boom-store-authorization.patch
    if [ "$RAVEIL_OWNED_CPU_MODE" = boom-store-token-handoff ] ||
       [ "$RAVEIL_OWNED_CPU_MODE" = boom-store-token-stripped-after-valid ]; then
      git -C "$build_root/chipyard/generators/boom" apply --check --unidiff-zero \
        /repo/hardware/chisel/chipyard-patches/t-0042-boom-store-token-handoff.patch
      git -C "$build_root/chipyard/generators/boom" apply --unidiff-zero \
        /repo/hardware/chisel/chipyard-patches/t-0042-boom-store-token-handoff.patch
      if [ "$RAVEIL_OWNED_CPU_MODE" = boom-store-token-stripped-after-valid ]; then
        git -C "$build_root/chipyard/generators/boom" apply --check --unidiff-zero \
          /repo/hardware/chisel/chipyard-patches/t-0042-boom-token-strip-after-valid.patch
        git -C "$build_root/chipyard/generators/boom" apply --unidiff-zero \
          /repo/hardware/chisel/chipyard-patches/t-0042-boom-token-strip-after-valid.patch
      fi
  ...
  if [ "$RAVEIL_OWNED_CPU_MODE" = boom-load-lifecycle ]; then
  ...
  elif [ "$RAVEIL_OWNED_CPU_MODE" = boom-store-token-handoff ] ||
       [ "$RAVEIL_OWNED_CPU_MODE" = boom-store-token-stripped-after-valid ]; then
    ...
    if [ "$RAVEIL_OWNED_CPU_MODE" = boom-store-token-stripped-after-valid ]; then
      [ "$(sha256sum "$build_root/chipyard/generators/boom/src/main/scala/lsu/mshrs.scala" | awk "{print \$1}")" = "2272df204127a91c175622fb7b7f1425854a8866e7f412913bcb897183fc8d3b" ]
    else
  ...
  install -D -m 0444 /repo/hardware/chisel/chipyard-overlay/RaveilFixtureInputProvider.scala \
    "$build_root/chipyard/generators/chipyard/src/main/scala/raveil/RaveilFixtureInputProvider.scala"
  install -D -m 0444 /repo/hardware/chisel/chipyard-overlay/RaveilStaticStencilCore.scala \
    "$build_root/chipyard/generators/chipyard/src/main/scala/raveil/RaveilStaticStencilCore.scala"
  install -D -m 0444 /repo/hardware/chisel/chipyard-overlay/RaveilStaticStencilTLClient.scala \
    "$build_root/chipyard/generators/chipyard/src/main/scala/raveil/RaveilStaticStencilTLClient.scala"
...
[ "$(sha256sum "$build_root/chipyard/generators/chipyard/src/main/scala/raveil/RaveilFixtureInputProvider.scala" | awk "{print \$1}")" = "$RAVEIL_FIXTURE_OVERLAY_SHA256" ]
[ "$(sha256sum "$build_root/chipyard/generators/chipyard/src/main/scala/raveil/RaveilStaticStencilCore.scala" | awk "{print \$1}")" = "$RAVEIL_STATIC_CORE_OVERLAY_SHA256" ]
[ "$(sha256sum "$build_root/chipyard/generators/chipyard/src/main/scala/raveil/RaveilStaticStencilTLClient.scala" | awk "{print \$1}")" = "$RAVEIL_STATIC_CLIENT_OVERLAY_SHA256" ]
[ "$(sha256sum "$build_root/chipyard/generators/rocket-chip/src/main/scala/rocket/HellaCache.scala" | awk "{print \$1}")" = "1672c56ad0cdaad15ac0184bf17193a5417bd949662793dec9cd1b8671cd8ad3" ]
[ "$(sha256sum "$build_root/chipyard/generators/rocket-chip/src/main/scala/tilelink/Bundles.scala" | awk "{print \$1}")" = "f1f8190de5064a50ac184749c965b09259748f7e20954fe2566cdfced9c41586" ]
if [ "$RAVEIL_OWNED_CPU_MODE" = rocket-postrequest-exception ]; then
  [ "$(sha256sum "$build_root/chipyard/generators/rocket-chip/src/main/scala/rocket/RocketCore.scala" | awk "{print \$1}")" = "f3015d47932074f79a6e78ca96c45de009a2f049b02ae3a1afa6f885b06774d8" ]
...
?? generators/chipyard/src/main/scala/raveil/RaveilFixtureInputProvider.scala
?? generators/chipyard/src/main/scala/raveil/RaveilOwnedTLMemory.scala
?? generators/chipyard/src/main/scala/raveil/RaveilStaticStencilCore.scala
?? generators/chipyard/src/main/scala/raveil/RaveilStaticStencilTLClient.scala"
...
test -f "$graph"
if [ "$RAVEIL_OWNED_CPU_MODE" = controlled-repeat ] ||
   [ "$RAVEIL_OWNED_CPU_MODE" = controlled-fixture-repeat ]; then
  ...
  repeated_c=/repo/hardware/chisel/riscv_stencil_repeated.c
  if [ "$RAVEIL_OWNED_CPU_MODE" = controlled-fixture-repeat ]; then
    repeated_c=/repo/hardware/chisel/riscv_stencil_fixture_repeated.c
  fi
  ...
  host_marker=RAVEIL-REPEATED-CPU-HOST-V1
  host_resource=16664d8ed96865c60ea41c91452b5e6748b055e0dfef3f786b13bd6f90127748
  if [ "$RAVEIL_OWNED_CPU_MODE" = controlled-fixture-repeat ]; then
    host_marker=RAVEIL-FIXTURE-CPU-HOST-V1
    host_resource=87be95fa8293da4b251675e9f81aea003e69e27ea6454a1d1db3c1611539e1f7
  fi
  printf "%s status=OK cpu=%s config=%s account=%s simulator_processes=1 resets=1 artifact_reloads=0 serialize_dispatch=%s source_sha256=%s artifact_sha256=%s toolchain_sha256=%s cache_source_sha256=%s build_input_sha256=%s resource_sha256=%s workload=frozen-rfc-0005 oracle=independent-host accounting=pending-completed-outer-raw-verification evidence=rtl-simulation-functional performance=not-measured\n" \
    "$host_marker" \
    "$RAVEIL_OWNED_CPU_LABEL" "$RAVEIL_OWNED_CPU_CONFIG_FQ" \
    "$RAVEIL_REPEAT_ACCOUNT" "$RAVEIL_CONTROLLED_SERIALIZE_DISPATCH" \
    "$RAVEIL_SOURCE_SHA256" "$artifact_sha256" "$RAVEIL_TOOLCHAIN_SHA256" \
    "$RAVEIL_CACHE_SOURCE_SHA256" "$RAVEIL_INPUT_SHA256" "$host_resource"
  exit 0
fi
...
if [ "$RAVEIL_OWNED_CPU_MODE" = boom-store-token-stripped-after-valid ]; then
  riscv64-unknown-elf-gcc \
    ...
    -T /repo/hardware/chisel/boom_functional_smoke.ld \
    /repo/hardware/chisel/owned_memory_boom_store_authorization.S \
    -o "$build_root/owned_memory_boom_store_token_stripped_after_valid.elf"
  boom_store_stripped_signature="$build_root/owned_memory_boom_store_token_stripped_after_valid.signature"
  boom_store_stripped_log="$build_root/owned_memory_boom_store_token_stripped_after_valid.log"
  rm -f "$boom_store_stripped_signature" "$boom_store_stripped_log"
  timeout --foreground 180 "$sim" +permissive +verbose \
    +signature="$boom_store_stripped_signature" +signature-granularity=4 +permissive-off \
    "$build_root/owned_memory_boom_store_token_stripped_after_valid.elf" 2>&1 | tee "$boom_store_stripped_log"
  python3 /repo/hardware/chisel/verify_owned_boom_store_authorization.py \
    "$boom_store_stripped_log" "$boom_store_stripped_signature"
  python3 /repo/hardware/chisel/verify_owned_boom_store_token_stripped_after_valid.py \
    "$boom_store_stripped_log"
  printf "OWNED-BOOM-STORE-TOKEN-STRIPPED-AFTER-VALID-HOST-V1 status=OK cpu=boom config=%s input_sha256=%s source_sha256=%s graph_sha256=%s patch_manifest=%s event_source=boom-pinned cpu_execution=rtl-simulation store_authorization=observed producer=valid-before-strip pre_strip_token=valid-epoch1-sequence1 post_strip_token=invalid-zero stripped_after_valid=observed token_classification=unknown-stripped semantic_attribution=not-promoted manager_put_a_d=observed manager_transaction=completed store_side_effect_readback=observed semantic_initiator=not-promoted general_stripped_metadata=not-proven malformed_nonzero=not-proven resource_match_verified=0 matched_comparison_ready=0 evidence=rtl-simulation-functional performance=not-measured\n" \
    "$RAVEIL_OWNED_CPU_CONFIG_FQ" "$RAVEIL_INPUT_SHA256" "$RAVEIL_SOURCE_SHA256" \
