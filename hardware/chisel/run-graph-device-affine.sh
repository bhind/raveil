...
artifact_parent=$repo_root/artifacts/graph_device_affine
case "$chisel_dir" in "$repo_root"/*) ;; *) exit 1;; esac
case "$artifact_parent" in "$repo_root"/artifacts/*) ;; *) exit 1;; esac

cleanup() {
  rm -rf "$chisel_dir/generated_static" \
    "$chisel_dir/generated_static_repeat" \
    "$chisel_dir/obj_graph_device_affine" \
...
cd "$repo_root"
python3 -m raveil.graph_device_affine prepare --output "$evidence_root"

dockerfile_sha256=$(shasum -a 256 "$chisel_dir/Dockerfile" | awk '{print $1}')
image="raveil-graph-device-affine:sha256-${dockerfile_sha256}"
label=$(docker image inspect \
  --format '{{index .Config.Labels "raveil.dockerfile.sha256"}}' \
...
test "$(docker image inspect \
  --format '{{index .Config.Labels "raveil.dockerfile.sha256"}}' \
  "$image")" = "$dockerfile_sha256"
image_id=$(docker image inspect --format '{{.Id}}' "$image")

{
  printf 'schema=raveil.graph-device-affine-environment/v1\n'
...
docker run --rm \
  ...
  "$image" \
  ./run-graph-device-affine-in-container.sh /evidence

python3 -m raveil.graph_device_affine finalize --evidence "$evidence_root"
printf 'GraphDevice-AFFINE-EVIDENCE-V1 path=artifacts/graph_device_affine/%s private=1 publication=0\n' \
