...
test "$first_parent" = "$second_parent" \
  ...
  && test "$(basename -- "$second")" = request-2 \
...
docker run --rm --network none --security-opt no-new-privileges=true --platform linux/amd64 \
  ...
  "/session/$(basename "$first")" "/session/$(basename "$second")" "$(basename "$session")" \
