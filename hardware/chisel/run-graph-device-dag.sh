...
artifact_parent=$repo_root/artifacts/graph_device_dag
...
cleanup() {
  rm -rf "$chisel_dir/generated_static" \
    ...
    "$chisel_dir/obj_graph_device_dag" \
...
cd "$repo_root"
python3 -m raveil.graph_device_dag prepare --output "$evidence_root"
...
{
  printf 'schema=raveil.graph-device-dag-environment/v1\n'
...
docker run --rm \
  ...
  "$image" \
  ./run-graph-device-dag-in-container.sh /evidence

python3 -m raveil.graph_device_dag finalize --evidence "$evidence_root"
printf 'GraphDevice-DAG-EVIDENCE-V1 path=artifacts/graph_device_dag/%s private=1 publication=0\n' \
