...
artifact_parent=$repo_root/artifacts/graph_device
case "$chisel_dir" in "$repo_root"/*) ;; *) exit 1;; esac
case "$artifact_parent" in "$repo_root"/artifacts/*) ;; *) exit 1;; esac

cleanup() {
  rm -rf "$chisel_dir/generated_static" "$chisel_dir/generated_graph_device" \
    "$chisel_dir/obj_graph_device" "$chisel_dir/.bsp" "$chisel_dir/.scala-build"
}
...
cd "$repo_root"
python3 -m raveil.graph_device_mvp prepare --output "$evidence_root"

dockerfile_sha256=$(shasum -a 256 "$chisel_dir/Dockerfile" | awk '{print $1}')
image="raveil-graph_device-sim:sha256-${dockerfile_sha256}"
...
{
  printf 'schema=raveil.graph-device-environment/v1\n'
...
docker run --rm \
  ...
  "$image" \
  ./run-graph-device-sim-mvp-in-container.sh /evidence

python3 -m raveil.graph_device_mvp finalize --evidence "$evidence_root"
printf 'GraphDevice-EVIDENCE-V1 path=artifacts/graph_device/%s private=1 publication=0\n' \
