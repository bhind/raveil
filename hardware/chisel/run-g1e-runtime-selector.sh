...
runner="$repo_root/hardware/chisel/run-g1e-runtime-selector.sh"
selector_verifier="$repo_root/hardware/chisel/verify_g1e_runtime_selector.py"
graph_verifier="$repo_root/hardware/chisel/verify_g1c_active_common_owned_memory.py"
rocket_verifier="$repo_root/hardware/chisel/verify_g1d_rocket_fallback_owned_memory.py"
...
RAVEIL_CHIPYARD_SOURCE="$chipyard" RAVEIL_ROCKET_CHIP_SOURCE="$rocket_source" \
...
input_sha256=$({
  shasum -a 256 \
    ...
    "$runner" "$selector_verifier" "$graph_verifier" "$rocket_verifier" \
    "$dockerfile" "$pin_file" \
    "$repo_root/hardware/chisel/riscv_integrated_graph_launch.c" \
    "$repo_root/hardware/chisel/riscv_stencil_fixture_repeated.c" \
    "$repo_root/hardware/chisel/riscv_stencil_repeated.S" \
    "$repo_root/hardware/chisel/riscv_stencil_system_scratchpad.ld" \
    "$repo_root/raveil/riscv_stencil_signature.py" \
    "$repo_root/raveil/static_region.py" |
    awk '{print $1}'
  printf 'chipyard_revision=%s\nrocket_revision=%s\n' \
    "$chipyard_revision" "$rocket_revision"
} | shasum -a 256 | awk '{print $1}')
input_prefix=$(printf '%s' "$input_sha256" | cut -c1-16)
build_volume="raveil-chipyard-g1e-runtime-$input_prefix"

printf 'G1E-RUNTIME-SELECTOR-HOST-V1 input_sha256=%s chipyard_revision=%s rocket_revision=%s build_volume=%s platform=%s evidence=rtl-simulation-functional performance=not-measured\n' \
  "$input_sha256" "$chipyard_revision" "$rocket_revision" \
  "$build_volume" "$platform"

docker run --rm --platform "$platform" --network none \
  --security-opt no-new-privileges=true \
  --env "RAVEIL_G1E_INPUT_SHA256=$input_sha256" \
  --env "RAVEIL_G1E_CHIPYARD_REVISION=$chipyard_revision" \
  --env "RAVEIL_G1E_ROCKET_REVISION=$rocket_revision" \
  ...
  --mount "type=bind,source=$selector_verifier,target=/identity/selector-verifier.py,readonly" \
  --mount "type=bind,source=$graph_verifier,target=/identity/graph-verifier.py,readonly" \
  --mount "type=bind,source=$rocket_verifier,target=/identity/rocket-verifier.py,readonly" \
...
expected_input="$RAVEIL_G1E_INPUT_SHA256"
expected_chipyard="$RAVEIL_G1E_CHIPYARD_REVISION"
expected_rocket="$RAVEIL_G1E_ROCKET_REVISION"
...
observed=$({
  sha256sum \
    ...
    /patches/t-0042-rocket-dcache-origin-hook.patch \
    /patches/t-0042-tlxbar-request-defaults.patch \
    /identity/runner.sh /identity/selector-verifier.py \
    /identity/graph-verifier.py /identity/rocket-verifier.py \
    /identity/Dockerfile.boom-sim /identity/boom-pin.env \
    /repo/hardware/chisel/riscv_integrated_graph_launch.c \
    /repo/hardware/chisel/riscv_stencil_fixture_repeated.c \
    /repo/hardware/chisel/riscv_stencil_repeated.S \
    /repo/hardware/chisel/riscv_stencil_system_scratchpad.ld \
    /repo/raveil/riscv_stencil_signature.py /repo/raveil/static_region.py |
    awk "{print \$1}"
  printf "chipyard_revision=%s\nrocket_revision=%s\n" \
...
[ -x /locked/env/riscv-tools/bin/riscv64-unknown-elf-gcc ]
[ -s /locked/env/riscv-tools/lib/libfesvr.a ]
[ "$(cat /locked/raveil-lock-sha256)" = "$expected_lock" ]

if [ ! -f /build/raveil-g1e-input-sha256 ]; then
  ...
  printf "%s\n" "$expected_input" > /build/raveil-g1e-input-sha256
else
  [ "$(cat /build/raveil-g1e-input-sha256)" = "$expected_input" ]
...
[ -s "$fir" ] && [ -s "$graphml" ]
grep -q "runtimeSelectionLocked" "$fir"
grep -q "runtimeGraphSelected" "$fir"
grep -q "runtimeLaunch" "$fir"
...
[ "$rocket_count" -gt 0 ] && [ "$client_count" -gt 0 ] && [ "$core_count" -gt 0 ]
[ "$manager_instances" -eq 1 ] && [ "$memory_definitions" -eq 1 ]
[ "$wrapper_count" -eq 0 ] && [ "$scratchpad_count" -eq 0 ]

rtl_list=/build/g1e-generated-rtl-files.txt
...
graph_elf=/build/riscv-stencil-g1e-graph.elf
rocket_elf=/build/riscv-stencil-g1e-rocket.elf
common_flags="-O2 -fno-strict-aliasing -march=rv64imafd_zicsr -mabi=lp64d -mcmodel=medany -nostdlib -nostartfiles -static -Wl,--no-relax"
riscv64-unknown-elf-gcc $common_flags \
  -T /repo/hardware/chisel/riscv_stencil_system_scratchpad.ld \
  /repo/hardware/chisel/riscv_stencil_repeated.S \
  /repo/hardware/chisel/riscv_integrated_graph_launch.c -o "$graph_elf"
riscv64-unknown-elf-gcc $common_flags \
  -DRFC0005_SYSTEM_SCRATCHPAD=1 -DRAVEIL_REPEAT_ACCOUNT=1 \
  -T /repo/hardware/chisel/riscv_stencil_system_scratchpad.ld \
  /repo/hardware/chisel/riscv_stencil_repeated.S \
  /repo/hardware/chisel/riscv_stencil_fixture_repeated.c -o "$rocket_elf"
graph_elf_sha256=$(sha256sum "$graph_elf" | awk "{print \$1}")
rocket_elf_sha256=$(sha256sum "$rocket_elf" | awk "{print \$1}")
graph_log=/build/riscv-stencil-g1e-graph.log
rocket_log=/build/riscv-stencil-g1e-rocket.log
rm -f "$graph_log" "$rocket_log"

printf "G1E-INTEGRATED-TOPOLOGY-V1 status=OK rocket=present graph_core=present graph_client=present runtime_selector=present runtime_modes=graph,rocket manager_instances=1 compared_data_sync_read_mem_definitions=1 graph_wrapper=absent local_scratchpad=absent rtl_files=%s rtl_tree_sha256=%s evidence=rtl-elaboration-topology performance=not-measured\n" \
  "$rtl_files" "$rtl_tree_sha256"
timeout --foreground 120 "$sim" +permissive +verbose +permissive-off "$graph_elf" 2>&1 | tee "$graph_log"
PYTHONPATH=/repo python3 /repo/hardware/chisel/verify_g1c_active_common_owned_memory.py < "$graph_log"
timeout --foreground 120 "$sim" +permissive +verbose +permissive-off "$rocket_elf" 2>&1 | tee "$rocket_log"
PYTHONPATH=/repo python3 /repo/hardware/chisel/verify_g1d_rocket_fallback_owned_memory.py --log "$rocket_log"
PYTHONPATH=/repo python3 /identity/selector-verifier.py \
  --graph-log "$graph_log" --rocket-log "$rocket_log"
printf "G1E-CLEAN-REPLAY-V1 status=OK input_sha256=%s chipyard_revision=%s rocket_revision=%s lock_sha256=%s simulator_sha256=%s graph_elf_sha256=%s rocket_elf_sha256=%s rtl_tree_sha256=%s evidence=rtl-simulation-functional performance=not-measured\n" \
  "$expected_input" "$expected_chipyard" "$expected_rocket" "$expected_lock" \
  "$sim_sha256" "$graph_elf_sha256" "$rocket_elf_sha256" "$rtl_tree_sha256"'
