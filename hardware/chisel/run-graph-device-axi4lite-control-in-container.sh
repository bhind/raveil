...
src=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd); build=/tmp/raveil-axi4lite-control
rm -rf "$build"; mkdir -p "$build/chipyard-overlay"
for f in OwnedFixedLatencyScratchpad.scala StaticStencilRegion.scala GraphDeviceAffineConfigInstaller.scala GraphDeviceProgramInstaller.scala GraphDeviceAxi4LiteTop.scala graph_device_axi4lite_control_verilator.cpp; do cp "$src/$f" "$build/"; done
...
cp /evidence/graph_device_axi4lite_aperture_generated.h "$build/"
cd "$build"
for f in \
...
{ printf '%s ' graph_device_axi4lite_aperture_generated.h; sha256sum graph_device_axi4lite_aperture_generated.h | awk '{print $1}'; } >> /tmp/source.manifest
{ printf '%s ' abi.sha256; sha256sum /evidence/abi.sha256 | awk '{print $1}'; } >> /tmp/source.manifest
...
# scala-cli does not follow chained using-file directives; make every elaboration
# input explicit so the receipt can bind the complete emitted design.
...
verilator --assert --cc generated_axi4lite/*.sv --exe graph_device_axi4lite_control_verilator.cpp --build --Mdir obj_graph_device_axi4lite --top-module GraphDeviceAxi4LiteTop > /evidence/verilator.stdout 2> /evidence/verilator.stderr
cp obj_graph_device_axi4lite/VGraphDeviceAxi4LiteTop /evidence/simulator.bin
sha256sum obj_graph_device_axi4lite/VGraphDeviceAxi4LiteTop | awk '{print $1}' > /evidence/simulator.sha256
{ scala-cli version; java -version 2>&1; verilator --version; } > /evidence/toolchain.txt
./obj_graph_device_axi4lite/VGraphDeviceAxi4LiteTop > /evidence/device.log
