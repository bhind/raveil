...
runner="$repo_root/hardware/chisel/run-g1d-rocket-fallback-owned-memory.sh"
verifier="$repo_root/hardware/chisel/verify_g1d_rocket_fallback_owned_memory.py"
...
build_volume="raveil-chipyard-g1d-fallback-$input_prefix"

printf 'G1D-ROCKET-FALLBACK-HOST-V1 input_sha256=%s chipyard_revision=%s rocket_revision=%s build_volume=%s platform=%s evidence=rtl-simulation-functional performance=not-measured\n' \
  "$input_sha256" "$chipyard_revision" "$rocket_revision" \
  "$build_volume" "$platform"

docker run --rm --platform "$platform" --network none \
  --security-opt no-new-privileges=true \
  --env "RAVEIL_G1D_INPUT_SHA256=$input_sha256" \
  --env "RAVEIL_G1D_CHIPYARD_REVISION=$chipyard_revision" \
  --env "RAVEIL_G1D_ROCKET_REVISION=$rocket_revision" \
...
expected_input="$RAVEIL_G1D_INPUT_SHA256"
expected_chipyard="$RAVEIL_G1D_CHIPYARD_REVISION"
expected_rocket="$RAVEIL_G1D_ROCKET_REVISION"
...
observed=$({
  sha256sum \
    ...
    /repo/raveil/riscv_stencil_signature.py \
    /repo/raveil/static_region.py |
    awk "{print \$1}"
  printf "chipyard_revision=%s\nrocket_revision=%s\n" \
...
if [ ! -f /build/raveil-g1d-input-sha256 ]; then
  ...
  printf "%s\n" "$expected_input" > /build/raveil-g1d-input-sha256
else
  [ "$(cat /build/raveil-g1d-input-sha256)" = "$expected_input" ]
...
config=RaveilFallbackIntegratedGraphRocketConfig
...
rtl_list=/build/g1d-generated-rtl-files.txt
...
elf=/build/riscv-stencil-g1d-fallback.elf
log=/build/riscv-stencil-g1d-fallback.log
riscv64-unknown-elf-gcc \
  ...
  /repo/hardware/chisel/riscv_stencil_fixture_repeated.c \
  -o "$elf"
[ "$(riscv64-unknown-elf-nm "$elf" | awk "\$3 == \"input_words\" { print \$1 }")" = 0000000008000000 ]
[ "$(riscv64-unknown-elf-nm "$elf" | awk "\$3 == \"output_words\" { print \$1 }")" = 0000000008000510 ]
sim_sha256=$(sha256sum "$sim" | awk "{print \$1}")
elf_sha256=$(sha256sum "$elf" | awk "{print \$1}")
rm -f "$log"

printf "G1D-INTEGRATED-TOPOLOGY-V1 status=OK rocket=present graph_core=present graph_client=present graph_selected=0 manager_instances=1 compared_data_sync_read_mem_definitions=1 graph_wrapper=absent local_scratchpad=absent rtl_files=%s rtl_tree_sha256=%s evidence=rtl-elaboration-topology performance=not-measured\n" \
  "$rtl_files" "$rtl_tree_sha256"
timeout --foreground 120 "$sim" +permissive +verbose +permissive-off \
  "$elf" 2>&1 | tee "$log"
PYTHONPATH=/repo python3 /repo/hardware/chisel/verify_g1d_rocket_fallback_owned_memory.py \
  --log "$log"
printf "G1D-CLEAN-REPLAY-V1 status=OK input_sha256=%s chipyard_revision=%s rocket_revision=%s lock_sha256=%s simulator_sha256=%s elf_sha256=%s rtl_tree_sha256=%s evidence=rtl-simulation-functional performance=not-measured\n" \
