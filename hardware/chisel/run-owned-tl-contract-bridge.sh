...
runner="$repo_root/hardware/chisel/run-owned-tl-contract-bridge.sh"
bridge_overlay="$repo_root/hardware/chisel/chipyard-overlay/RaveilOwnedTLContractBridge.scala"
driver="$repo_root/hardware/chisel/owned_tl_contract_bridge_sim_main.cpp"
dockerfile="$repo_root/hardware/chisel/Dockerfile.owned-tl"
image=raveil-owned-tl-protocol:v1
platform=linux/amd64

for input in "$runner" "$bridge_overlay" "$driver" "$dockerfile"; do
    [ -f "$input" ] || {
        echo "error: required owned TL contract bridge input is missing: $input" >&2
        exit 1
    }
done
[ -d "$chipyard/.git" ] || {
...
input_sha256=$(
    shasum -a 256 "$runner" "$bridge_overlay" "$driver" "$dockerfile" |
        awk '{print $1}' |
        shasum -a 256 |
        awk '{print $1}'
)
overlay_sha256=$(shasum -a 256 "$bridge_overlay" | awk '{print $1}')
...
docker run --rm \
    ...
    --mount "type=bind,source=$bridge_overlay,target=/overlay/RaveilOwnedTLContractBridge.scala,readonly" \
    --mount "type=bind,source=$driver,target=/overlay/owned_tl_contract_bridge_sim_main.cpp,readonly" \
    ...
    --mount type=volume,source=raveil-owned-tl-contract-bridge-assembly-v1,target=/assembly-cache \
...
cd /work/chipyard
install -D -m 0444 /overlay/RaveilOwnedTLContractBridge.scala \
  generators/chipyard/src/main/scala/raveil/RaveilOwnedTLContractBridge.scala
...
output=/work/generated-owned-tl-contract-bridge
mkdir -p "$output"
java -Xmx8G -cp "$assembly" chipyard.Generator \
  --target-dir "$output" \
  --name chipyard.raveil.RaveilOwnedTLContractBridgeHarness \
  --top-module chipyard.raveil.RaveilOwnedTLContractBridgeHarness \
...
[ -s "$fir" ] || { echo "error: contract bridge harness FIRRTL was not emitted" >&2; exit 1; }
[ -s "$anno" ] || { echo "error: contract bridge harness annotations were not emitted" >&2; exit 1; }
verilog="$output/RaveilOwnedTLContractBridgeHarness.v"
java -Xmx8G -cp "$assembly" firrtl.stage.FirrtlMain \
  ...
  -faf "$anno" \
  -X verilog
[ -s "$verilog" ] || { echo "error: contract bridge harness Verilog was not emitted" >&2; exit 1; }
verilator --assert --cc "$verilog" \
  generators/rocket-chip/src/main/resources/vsrc/plusarg_reader.v \
  --exe /overlay/owned_tl_contract_bridge_sim_main.cpp \
  --build \
  --Mdir "$output/verilator" \
  --top-module RaveilOwnedTLContractBridgeHarness \
  -CFLAGS "-std=c++17 -Wall -Wextra -Werror"
"$output/verilator/VRaveilOwnedTLContractBridgeHarness"'
