...
{ scala-cli version; java -version 2>&1; verilator --version; } > /tmp/dynamic-toolchain.txt
reuse=0
if test -e /session/build-dependencies.sha256; then
  test -f /session/build-dependencies.sha256 && test ! -L /session/build-dependencies.sha256
  dependency_before=$(bash "$src/build-cache-dependencies.sh")
  test "$dependency_before" = "$(cat /session/build-dependencies.sha256)"
fi
if test -e /session/reuse-build || test -L /session/reuse-build; then
  test -d /session/reuse-build && test ! -L /session/reuse-build
  test -n "${dependency_before:-}"
  for file in simulator.bin source.manifest rtl.manifest abi.manifest toolchain.txt identity.json bundle.sha256; do
    test -f "/session/reuse-build/$file" && test ! -L "/session/reuse-build/$file"
  done
  (cd /session/reuse-build && sha256sum --strict --status -c bundle.sha256)
  cmp /session/reuse-build/identity.json /session/build-identity.json
  cmp /session/reuse-build/source.manifest /tmp/dynamic-source.manifest
  cmp /session/reuse-build/abi.manifest /tmp/dynamic-abi.manifest
  cmp /session/reuse-build/toolchain.txt /tmp/dynamic-toolchain.txt
  cp /session/reuse-build/simulator.bin ./reused-simulator
  chmod 700 ./reused-simulator
  cp /session/reuse-build/rtl.manifest /tmp/dynamic-rtl.manifest
  simulator=./reused-simulator
  reuse=1
else
...
for root in "$first" ${second:+"$second"}; do
  ...
  cp -n /tmp/dynamic-toolchain.txt "$root/toolchain.txt"
done
for root in "$first" ${second:+"$second"}; do
...
if test -n "${dependency_before:-}"; then
  test "$dependency_before" = "$(bash "$src/build-cache-dependencies.sh")"
fi
if test "$request_count" = 1; then invocation=once; else invocation=twice; fi
if test "$reuse" = 1; then
printf 'GraphDevice-AXI4LITE-DYNAMIC-EVIDENCE-V2 status=PASS requests=%s same_simulator=1 invoked_%s=1 rtl_emitted_once=0 simulator_built_once=0 build_reused=1 rejected_before_axi=1 simulator_sha256=%s path=artifacts/graph_device_axi4lite_dynamic/%s evidence=rtl-simulation-functional performance=not-measured\n' "$request_count" "$invocation" "$simulator_sha" "$session_name"
else
