#!/usr/bin/env python3
"""Verify BOOM-local store authorization and separate manager A/D fate."""
...
def fail(message: str) -> "None":
    raise SystemExit(f"BOOM store authorization verification failed: {message}")
...
def main() -> None:
    if len(sys.argv) != 3:
        fail("usage: verify_owned_boom_store_authorization.py LOG SIGNATURE")
    ...
    print(
        "BOOM-STORE-AUTHORIZATION-V1 status=OK event_source=boom-pinned "
        ...
        "manager_put_a_d=independently-observed manager_a_d_source_correlation=observed "
        "transport_token_correlation=not-carried store_attribution=not-proven "
        "semantic_initiator=not-proven general_store_lifecycle=not-proven "
