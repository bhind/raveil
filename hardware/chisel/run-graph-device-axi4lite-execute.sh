...
artifact_parent=$repo_root/artifacts/graph_device_axi4lite_execute
mkdir -p "$artifact_parent"
evidence_root=$(mktemp -d "$artifact_parent/run.XXXXXX")
trap 'rm -rf "$repo_root/hardware/chisel/generated_axi4lite" "$repo_root/hardware/chisel/generated_axi4lite_repeat" "$repo_root/hardware/chisel/obj_graph_device_axi4lite_execute"' EXIT HUP INT TERM
...
cd "$repo_root"
python3 -m raveil.graph_device_axi4lite_execute prepare --output "$evidence_root"
printf 'schema=raveil.graph-device-axi4lite-execute-environment/v1\nplatform=linux/amd64\nimage_id=%s\n' "$image_id" > "$evidence_root/environment.txt"
docker run --rm --network none --security-opt no-new-privileges=true --platform linux/amd64 \
  ...
  --workdir /repo/hardware/chisel "$image_id" ./run-graph-device-axi4lite-execute-in-container.sh /evidence > "$evidence_root/container.stdout" 2> "$evidence_root/container.stderr"
python3 -m raveil.graph_device_axi4lite_execute finalize --evidence "$evidence_root"
python3 -m raveil.graph_device_axi4lite_execute verify --evidence "$evidence_root" >/dev/null
printf 'GraphDevice-AXI4LITE-EXECUTE-EVIDENCE-V1 path=artifacts/graph_device_axi4lite_execute/%s private=1 publication=0\n' "$(basename "$evidence_root")"
