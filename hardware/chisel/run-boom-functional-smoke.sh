...
runtime_image_id=$("$repo_root/hardware/chisel/verify-boom-functional-sim-image.sh")

printf 'BOOM-SIM-TOOLCHAIN-HOST-V1 runtime_image_id=%s platform=%s lock_sha256=%s conda_lock_bootstrap=unlocked cache=evidence-neutral performance=not-measured\n' \
    "$runtime_image_id" "$platform" "$lock_sha"
