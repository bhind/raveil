...
src=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd); build=/tmp/raveil-axi4lite-execute
rm -rf "$build"; mkdir -p "$build/chipyard-overlay"
for f in OwnedFixedLatencyScratchpad.scala StaticStencilRegion.scala GraphDeviceAffineConfigInstaller.scala GraphDeviceProgramInstaller.scala GraphDeviceAxi4LiteTop.scala graph_device_axi4lite_execute_verilator.cpp; do cp "$src/$f" "$build/"; done
...
for f in graph_device_abi_generated.h graph_device_axi4lite_aperture_generated.h graph_device_axi4lite_execute_vectors.h; do cp "/evidence/$f" "$build/"; done
cd "$build"
for f in \
...
for f in abi.sha256 graph_device_abi_generated.h graph_device_axi4lite_aperture_generated.h graph_device_axi4lite_execute_vectors.h input-seed-1.bin input-seed-2.bin input-seed-3.bin oracle-seed-1.bin oracle-seed-2.bin; do
...
verilator --assert --cc generated_axi4lite/*.sv --exe graph_device_axi4lite_execute_verilator.cpp --build --Mdir obj_graph_device_axi4lite_execute --top-module GraphDeviceAxi4LiteTop > /evidence/verilator.stdout 2> /evidence/verilator.stderr
cp obj_graph_device_axi4lite_execute/VGraphDeviceAxi4LiteTop /evidence/simulator.bin
sha256sum obj_graph_device_axi4lite_execute/VGraphDeviceAxi4LiteTop | awk '{print $1}' > /evidence/simulator.sha256
{ scala-cli version; java -version 2>&1; verilator --version; } > /evidence/toolchain.txt
./obj_graph_device_axi4lite_execute/VGraphDeviceAxi4LiteTop /evidence > /evidence/device.log 2> /evidence/device.stderr
