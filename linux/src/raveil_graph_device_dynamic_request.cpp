...
void validate_program(const std::array<std::uint32_t, 32>& payload, std::uint32_t request_version) {
    ...
    for (std::size_t index = 0; index < 16; ++index) {
        ...
        const bool relative_load = (program_version >= kVersionV3 && program_version <= 5U)
        ...
        const bool max_u32 = opcode == 4U
            && (program_version >= kVersionV2 && program_version <= 5U)
            && (instruction & 0x0007ffffU) == 0U && defined[source_a] && defined[source_b];
        const bool mul_u32 = opcode == 5U && (program_version == kVersionV4 || program_version == 5U)
            && (instruction & 0x0007ffffU) == 0U && defined[source_a] && defined[source_b];
        const bool immediate = opcode == 6U && program_version == 5U && defined[source_a];
        const bool store = opcode == 3U && (instruction & 0x01ffffffU) == 0U
            && defined[destination] && index == payload[2] - 1U;
        if (active && !(load || add || max_u32 || mul_u32 || immediate || store)) throw std::runtime_error("dynamic program instruction is invalid");
        if (!active && instruction != 0U) throw std::runtime_error("dynamic program padding is nonzero");
        if (load || add || max_u32 || mul_u32 || immediate) defined[destination] = true;
        if (store) ++stores;
    }
...
DynamicGraphDeviceRequest read_dynamic_graph_device_request(const std::filesystem::path& root) {
    ...
    if (word(bytes.data()) != kMagic || (request_version != kVersionV1
        && request_version != kVersionV2 && request_version != kVersionV3 && request_version != kVersionV4 && request_version != kVersionV5 && request_version != kVersionV6)
    ...
    if ((request_version == kVersionV5 || request_version == kVersionV6) && result.seed != 0U)
        throw std::runtime_error("explicit dynamic input must use seed slot zero");
    if (request_version != kVersionV5 && request_version != kVersionV6) validate_input(result.input, result.seed);
