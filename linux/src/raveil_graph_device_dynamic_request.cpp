...
constexpr std::uint32_t kVersionV7 = 7U;
...
void validate_program(const std::array<std::uint32_t, 32>& payload, std::uint32_t request_version) {
    ...
    if (payload[0] != 0x52504731U || (request_version == kVersionV5
            ...
            : request_version == kVersionV6 ? program_version != 5U
            : request_version == kVersionV7 ? program_version != 6U
    ...
    for (std::size_t index = 0; index < 16; ++index) {
        ...
        const bool signed_unit_column = column_bits == 0U || column_bits == 1U
            || column_bits == 31U;
        const bool legacy_load = program_version < kVersionV3 && selector <= 4U
            && (instruction & 0x003fffffU) == 0U;
        const bool relative_load = (program_version >= kVersionV3 && program_version <= 6U)
        ...
        const bool max_u32 = opcode == 4U
            && (program_version >= kVersionV2 && program_version <= 6U)
            && (instruction & 0x0007ffffU) == 0U && defined[source_a] && defined[source_b];
        const bool mul_u32 = opcode == 5U && (program_version == kVersionV4 || program_version == 5U || program_version == 6U)
            && (instruction & 0x0007ffffU) == 0U && defined[source_a] && defined[source_b];
        const bool immediate = opcode == 6U && (program_version == 5U || program_version == 6U) && defined[source_a];
        const bool comparison = opcode == 7U && program_version == 6U && defined[source_a];
        const bool store = opcode == 3U && (instruction & 0x01ffffffU) == 0U
            && defined[destination] && index == payload[2] - 1U;
        if (active && !(load || add || max_u32 || mul_u32 || immediate || comparison || store)) throw std::runtime_error("dynamic program instruction is invalid");
...
DynamicGraphDeviceRequest read_dynamic_graph_device_request(const std::filesystem::path& root) {
    ...
    if (word(bytes.data()) != kMagic || (request_version != kVersionV1
        && request_version != kVersionV2 && request_version != kVersionV3 && request_version != kVersionV4 && request_version != kVersionV5 && request_version != kVersionV6 && request_version != kVersionV7)
    ...
    if ((request_version == kVersionV5 || request_version == kVersionV6 || request_version == kVersionV7) && result.seed != 0U)
        throw std::runtime_error("explicit dynamic input must use seed slot zero");
    if (request_version != kVersionV5 && request_version != kVersionV6 && request_version != kVersionV7) validate_input(result.input, result.seed);
