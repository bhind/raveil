...
class PhysicalProxyToolchainTests(unittest.TestCase):
    ...
    def test_frozen_stage_b_manifest_is_exact(self) -> None:
        ...
        recovery_v8_path = (
            ROOT / "benchmarks/manifests/t0044-static-physical-screen-recovery-v8.json"
        )
        recovery_v8 = t0044_physical.load_manifest(recovery_v8_path)
        self.assertEqual(
            recovery_v8["recovery_of_manifest_sha256"],
            t0044_physical.sha256_file(recovery_v7_path),
        )
        self.assertEqual(
            recovery_v8["operational_change"],
            "verify-rtl-before-collection-and-mount-reverified-private-snapshot",
        )
        self.assertEqual(
            recovery_v8["compatibility_lowering_policy"],
            recovery_v7["compatibility_lowering_policy"],
        )
    ...
    def test_candidate_runner_is_manifest_driven_and_separates_evidence(self) -> None:
        ...
        self.assertIn("physical lowering changed normalized annotation", rocket_export)
        self.assertIn("physical lowering changed module hierarchy", rocket_export)
        self.assertIn("<build-root>", rocket_export)
        self.assertIn("LC_ALL=C sort", rocket_export)
