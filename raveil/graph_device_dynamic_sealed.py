...
def _valid_program_words(words: list[int]) -> bool:
    ...
    if len(words) != PROGRAM_WORDS or words[0] != 0x52504731 or words[1] != 1 or words[3] != 8:
        return False
    count = words[2]
    if not 1 <= count <= 16 or any(words[index] for index in range(12 + count, 32)):
        return False
    defined: set[int] = set()
...
def _materialize_verified(verified: dict[str, Any], destination: Path) -> None:
    """Materialize only verified in-memory payloads into a new private root.

    This deliberately never reads or parses ``descriptor.json`` after ``verify``.
    """
    if destination.exists() or destination.is_symlink():
        raise GraphDeviceDynamicSealError("replay destination must not already exist")
    try:
        destination.mkdir(mode=0o700)
        directory = os.open(destination, os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0))
    except OSError as error:
        raise GraphDeviceDynamicSealError("replay destination cannot be exclusively created") from error
    try:
        ...
        inputs = os.open("inputs", os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0), dir_fd=directory)
        try:
            info = os.fstat(inputs)
            if not stat.S_ISDIR(info.st_mode):
                raise GraphDeviceDynamicSealError("replay inputs directory is unsafe")
        ...
        _write_at(directory, "seal-binding.json", (json.dumps({
            "schema": SCHEMA.replace("sealed", "sealed-replay"),
            "seal_sha256": verified["seal_sha256"], "manifest_sha256": verified["manifest_sha256"],
            "request_sha256": verified["manifest"]["request_sha256"],
...
def replay(bundle: Path, destination: Path, repository: Path) -> dict[str, Any]:
    verified = verify(bundle, repository)
    _materialize_verified(verified, destination)
    return verified
...
def run_sealed(bundle: Path, repository: Path) -> dict[str, Any]:
    ...
    root = _dynamic_parent(repo)
    if root.is_symlink() or (root.exists() and not root.is_dir()):
        raise GraphDeviceDynamicSealError("dynamic replay artifact root is unsafe")
    root.mkdir(parents=True, exist_ok=True)
    session = Path(tempfile.mkdtemp(prefix="run.", dir=root))
    request_root = session / "request-1"
    _materialize_verified(verified, request_root)
    ...
    for role, name in (("output", f"private-output-{graph_id}-seed-{seed}.bin"), ("fallback", f"fallback-output-{graph_id}-seed-{seed}.bin")):
        outputs[role] = _read_evidence(request_root / name)
    ...
    for name in ("source.manifest", "abi.manifest", "rtl.manifest", "toolchain.txt", "simulator.bin", "simulator.sha256", "axi-transcript.log", "device.log"):
        ...
        try:
            ...
            evidence_bytes[name] = _read_evidence(path, limit)
    ...
    _write_new(request_root / "sealed-dynamic-receipt.json", (json.dumps(receipt, sort_keys=True) + "\n").encode("ascii"))
    return receipt
