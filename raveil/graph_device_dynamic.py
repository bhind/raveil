...
def _request_bytes(program: dict[str, Any], profile: dict[str, Any], seed: int,
    ...
    if version >= 5 and input_bytes is None:
        raise GraphDeviceDynamicError(f"program v{version} requires explicit snapshot input")
    header = struct.pack(
        "<8I", MAGIC, version, HEADER_BYTES, 0 if name == "baseline" else 1,
        seed, PROGRAM_WORDS, AFFINE_WORDS, INPUT_WORDS,
    )
    if input_bytes is not None:
        header = struct.pack("<8I", MAGIC, version + 1 if version >= 5 else 5, HEADER_BYTES, 0 if name == "baseline" else 1,
...
def prepare_request(output: Path, graph: str, seed: int, repository: Path | None = None,
    ...
    if input_bytes is not None:
        metadata.update({"schema": f"raveil.graph-device-dynamic-request/v{program['payload'][1] + 1 if program['payload'][1] >= 5 else 5}",
