...
- [x] **T-0193** Preview and selectively export saved-run files, Issue #255,
  ...
  Technically accepted for integration: 76 tests pass, independent audit passes,
  actual selected-file demo retains 2 bytes and omits 19 unrelated members.
  No Drive upload as part of this task.

- [x] **T-0194** Inspect selected-file derivatives without extraction, Issue #256.
  Technically accepted for integration after T-0193, S-0005, 3 SP. Validate bounded envelope,
  payload lengths and hashes; list selected members without extracting files,
  executing, importing a run or inferring authenticity. All 85 focused/regression
  tests pass; independent security review GO and actual export inspection pass.

- [ ] **T-0184** Choose one bounded capacity follow-up from real compiler
  pressure, Issue #183. Planning successor after T-0194; T-0185 and T-0155
  dependencies are complete. Preserve rejection fixtures for instruction,
  live-register, shape/stride and halo limits; compare schedule/reuse/tiling
  without changing code or machine bounds. At most one proposed next slice.
...
- [x] **T-0180** Specify a bounded saved-run export boundary, Issue #178,
  ...
  review informed directory-budget correction. Owner authorized the separate
  preview/selective-export successor. No runtime export/import in this task.
...
- [x] **T-0185** Apply bounded Graphs to real grid-processing tasks,
  ...
  Exact commits `2f20d3a` and review correction `0bad97c` passed 825 tests,
  three actual RTL runs, independent review and Vreji audit. PR #233 merged as
  `e2192ae`; Issue #184 closed and canonical Project Done was verified.
...
- [x] **T-0179** Explain recipe readiness before launching a backend,
  ...
  Host Functional. Exact `e8bdd68` passed 820 tests plus security and Vreji
  review; PR #231 merged as `3537b74`, Issue #177 closed and Project Done was
  verified. It invokes no workload tool or backend and Docker daemon/image
  readiness remains explicitly not checked.
...
- [x] **T-0178** Fork an editable recipe without overwriting its source,
  ...
  Exact `9118fa4` plus atomic-publication fix `6b85bd7` passed Vreji and
  primary review; PR #229 merged as `85e17d6`, Issue #176 closed and canonical
...
- [x] **T-0162** Clarify delegated task acceptance under ADR-0094,
  ...
  handling without runtime permission changes. Record and diff checks passed;
  independent exact-candidate audit found no material concern and delegated
...
|---|---|---|
| **P0 — immediate** | At most two explicitly independent delivery lanes may run under ADR-0061, ADR-0065, and ADR-0066. | T-0194 technical integration; T-0193 is integrated. |
| **P1 — next** | Start only after its named dependencies pass. | T-0184: planning-only Graph rejection fixtures/report, S-0005, 3 SP, after T-0194. No capacity/RTL/ISA change or real-device successor is authorized; KV260 remains gated by T-0138 and ADR-0039. |
...
- [x] **T-0146** Keep a rolling P0 delivery horizon instead of treating an
  ...
  `fix/t-0146-p0-auto-replenishment`, Project Manager, five SP, Sprint S-0002,
  and Host Functional governance evidence own the canonical `prepare`
  transition, pullable-Ready horizon audit, Sprint operator correction,
  regressions, ADR-0082 and matching records. Preserve the two-item mutation
  limit, technical integration continuity, weekly usage/HCI boundaries, and
  Project Manager authority. Stop before product implementation, automatic
  material strategy decisions, hardware/license/cost actions, performance or
  research claims. T-0145 is the bounded successor candidate; T-0146 must
  prepare it as P1/Ready before closeout. Accepted on the task revision by 70
...
- [x] **T-0132/S01** Prove the smallest repository-owned AXI4-Lite control
  ...
  unbased relative 16 KiB aperture for the byte-identical execution, affine-
  installation, and program-installation ABIs. S01 exposes identity, version,
  status, counts, and execution reset only; all data/install windows remain
  rejected for later slices.

  Real Issue #55, branch `feat/t-0132-s01-axi4lite-control`, Systems
  Implementer, five SP, and Sprint S-0001 own the seven implementation paths;
  the Project Manager owns ADR and records. A candidate checkpoint preceded
  the required ADR and had no authority. After ADR-0067 acceptance, the
  corrected candidate at `363cad3` enforces one total outstanding transaction,
  independent AW/W capture, fail-closed response classes, exact install-status
  bits, external ARESETn clearing, and a B-handshake-preserving core reset
  barrier. Thirty-six focused/regression tests and the actual offline Chisel/
  Verilator operator demo pass. The receipt binds byte-identical ABI hashes,
  identical double-emitted RTL manifests, the simulator, sources, toolchain,
  environment, and exact device marker. This completes S01 only as
...
- [x] **T-0132/S03** Carry one complete factory five-point invocation through
  ...
  Performance reviews pass. Both reviewers approved exact head `6cf910e`; PR
...
- [x] **T-0132/S04** Connect the existing three-Graph operator/runtime to the
  ...
  local candidate on `feat/t-0132-s04-axi-selected`. The owner later authorized
  continuous integration; Issue #81 and PR #82 integrated the ordered
  S04--S07 stack as canonical merge `276469a`.
  The implementation passes primary RTL simulation, but its CLI option is
...
- [x] **T-0132/S05** Connect one admitted `--graph`/`--seed` request to the
  ...
  stacked candidate on `feat/t-0132-s05-axi-request` based on reviewed S04
  commit `0b248ed`. Issue #81 and PR #82 later integrated it without changing
  its evidence or authority boundary.

- [x] **T-0132/S06** Reuse the S05 transport-neutral runtime through one Linux
  ...
  Graphs, Experience, or evidence promotion. The slice was developed on
  `feat/t-0132-s06-linux-uio-transport` and later integrated by Issue #81 and
  PR #82 without gaining an FPGA claim. The accepted implementation
  ...
  `run.Glve09`. No real UIO/FPGA run exists.

- [x] **T-0132/S07** Export the current `GraphDeviceAxi4LiteTop` as one
  ...
  FPGA resources/timing, or hardware success. The implementation on
...
- [x] **T-0131** Make the existing weekly Sprint governance repeatable as a
  ...
  sole queue-transition implementation; and returns one stable authority,
  usage, Project, evidence, HCI, and next-action receipt. Its detailed
  reference preserves Initial SP, executable demos, Definition of Ready/Done,
  canonical records, evidence classes, and Keep/Problem/Try without creating a
...
- [x] **T-0130** Make GitHub Project #1 truthful as both the executable queue
  ...
  Reconcile the Project README and live item count through T-0129, move the
  non-pullable S-0001 review Draft from Ready to Backlog, and provide a clean
  current-main operator workspace without modifying the dirty T-0042 root.
  Real Issue #48, branch `chore/t-0130-project-hygiene`, Project Manager, three
...
- [x] **T-0128** Turn the accepted three-Graph device capability into a small,
  ...
  S02 was promoted under real Issue #39 and branch
  ...
  program. The implementation passes 71 focused host tests. Primary private RTL runs
